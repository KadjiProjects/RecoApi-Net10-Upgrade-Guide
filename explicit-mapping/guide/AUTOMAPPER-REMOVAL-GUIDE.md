# Replacing AutoMapper with ExplicitMapping — a generic, LLM-executable guide
...
> **This guide is scoped to exactly one thing** — removing AutoMapper and replacing it
> with the `ExplicitMapping` library — and is written to apply to **any** .NET project,
> on any target framework, whether or not that project is also being upgraded to a newer
> .NET version.
...
`ExplicitMapping` — a small, dependency-free library of mapping contracts and a test-time
coverage verifier. Its complete, buildable source lives in this repository at
[`../src/ExplicitMapping/`](../src/ExplicitMapping/) (7 files, zero package dependencies), with
a working demo/test project at [`../tests/ExplicitMapping.Tests/`](../tests/ExplicitMapping.Tests/)
you can build and run right now with `dotnet build ../ExplicitMapping.sln && dotnet test
../ExplicitMapping.sln` to see it work before touching your own project.
...
- [§2 Add the library](#add-the-library) — bringing ExplicitMapping in; the pre-C#11 fallback · GATE 1
...
- Every place that used to call `_mapper.Map<T>(x)`, `_mapper.Map(x, existing)`, or
  ...
  a `ExplicitMapping` interface, or a plain static/extension method, or (for LINQ
  `ProjectTo` call sites specifically) a compiled `Expression<Func<TSource,TDest>>`.
- Every mapping pair has at least one test built on `ExplicitMapping.Verification.
...
1. **DO NOT** introduce a different mapping library (Mapperly, TinyMapper, a hand-rolled
   reflection-based mapper) as the replacement. The target is `ExplicitMapping` (or, where
...
`ExplicitMapping`'s four interface files (`IMappedFrom`, `IMapsTo`, `IScalarCopyable`,
...
| --- | --- |
| Target framework is .NET 7, 8, 9, 10, or any framework buildable with a .NET 7+ SDK and `<LangVersion>11</LangVersion>` or later | **Interface tier** — use all four interfaces as shipped in [`../src/ExplicitMapping/`](../src/ExplicitMapping/). |
...
## 2. Add ExplicitMapping to your project
...
Copy the folder [`../src/ExplicitMapping/`](../src/ExplicitMapping/) from this repository into
your solution — e.g. `src/Common/ExplicitMapping/` if you have a shared-libraries
...
```bash
dotnet sln <your-solution>.sln add path/to/ExplicitMapping/ExplicitMapping.csproj
dotnet add <each consuming project>.csproj reference path/to/ExplicitMapping/ExplicitMapping.csproj
```

### 2.2 Interface tier (C# 11 / .NET 7+) — use everything as-is

Retarget the copied `ExplicitMapping.csproj`'s `<TargetFramework>` from `net10.0` to match
...
3. **Keep unchanged**: `ExplicitMapping.csproj` (retarget `<TargetFramework>` to match your
...
```bash
dotnet build path/to/ExplicitMapping/ExplicitMapping.csproj
```
...
`ExplicitMapping` interface, so it works identically for both library tiers and for
DI-dependent mapping services (§5.1). Add the test project's reference to `ExplicitMapping`
(and, if separate, `ExplicitMapping.Verification`'s namespace — it lives in the same
...
- [ ] **Library is in place (§2)**: correct tier chosen per project; `ExplicitMapping`
...
```
Replace AutoMapper with ExplicitMapping
...
for the same content split across navigable files, or
[`../src/ExplicitMapping/`](../src/ExplicitMapping/) for the library's buildable source.
