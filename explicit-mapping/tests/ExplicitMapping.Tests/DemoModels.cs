...
/// persistence model looks like. Implements <see cref="IDualMapped{TSelf, TOther}"/> in
/// full: bidirectional mapping plus scalar clone/copy for the repository update path
/// AutoMapper used to power via <c>_mapper.Map(dbModel, currentRecord)</c>.
/// </summary>
public sealed class PersonEntity : IDualMapped<PersonEntity, PersonDto>
{
    ...
    public void CopyScalarsTo(PersonEntity target)
    {
        ...
        // AutoMapper's in-place Map(dbModel, currentRecord) in a repository's Update method.
    }
}
